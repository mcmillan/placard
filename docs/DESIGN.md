...
| Command | Effect |
|---|---|
| `show { text, bg?, fg?, flash? }` | Arbitrary text; colours default to current; `flash` inverts fg/bg every 500 ms for a fixed time or until the next command |
...
enum Command {
    Show          { text: String, bg: Option<Rgb>, fg: Option<Rgb>, flash: Flash },
    Canned        { id: String, bg: Option<Rgb>, fg: Option<Rgb>, flash: Option<Flash> },
...
text = "SHOW STOP"
bg = "8a0000"
flash = { infinite = true }   # true = 3 s; or a number of seconds; or infinite
